...
class tc_date extends tc_date_main {
    ...
    function addDay($timespan, $format = "Y-m-d", $cdate = "") {
        if (!$this->compatible) {
            return tc_date_main::addDay($timespan, $format, $cdate);
        } else {
            $timespan = "P" . $timespan . "D";
            return $this->addDate($timespan, $format, $cdate);
        }
    }

    function addMonth($timespan, $format = "Y-m-d", $cdate = "") {
        if (!$this->compatible) {
            return tc_date_main::addMonth($timespan, $format, $cdate);
        } else {
    ...
    function addYear($timespan, $format = "Y-m-d", $cdate = "") {
        if (!$this->compatible) {
            return tc_date_main::addYear($timespan, $format, $cdate);
        } else {
    ...
    function addDate($timespan, $format = "Y-m-d", $cdate = "") {
        if ($this->compatible) {
            $tmp_date = ($cdate != "") ? new DateTime($cdate) : $this->mydate;
    ...
    function validDate($date) {
        if (!$this->compatible) {
            return tc_date_main::validDate($date);
        } else {
            try {
