...
<?php
$mois = mysqli_real_escape_string($linki, $_POST['mois']);
$annee = mysqli_real_escape_string($linki, $_POST['annee']);
$service = mysqli_real_escape_string($linki, $_POST['service']);

$sql = "SELECT * FROM $tbl_appaut WHERE MONTH(date)=$mois AND YEAR(date)=$annee AND service='$service' ORDER BY idapp_aut DESC";
...
$tPT = $AFPT['montant'];
?>
<a href="app_aut_affichage_imp.php?a=<?php echo md5(microtime()).$mois; ?>&b=<?php echo md5(microtime()).$annee; ?>&c=<?php echo md5(microtime()).$service; ?>" target="_blank">
