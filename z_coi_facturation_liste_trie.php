   ...
   for ($cpt = $cpt_deb; $cpt <= $cpt_fin; $cpt++) { 
      if ($cpt == $page_active) { 
         ...
         if ($cpt == $cpt_fin) { 
            ...
            $barre .= "&etat=$valeuretat&annee=$ARCH'>".$cpt."</A>"; 
         } 
         else { 
...
?>
     ...
     <td width="49%"><div class="panel panel-warning">
       <div class="panel-heading">
         <h3 class="panel-title">ARCHIVE : Trie par Statut</h3>
       </div>
       <div class="panel-body">
         <table width="100%" border="0" cellpadding="0" cellspacing="0" bordercolor="#000000">
           <tr>
             <td width="47%"><table width="100%" border="0.5" cellspacing="0" cellpadding="0">
               <tr>
                 <td width="52%"><form action="z_coi_facturation_liste_trie.php" method="post" name="form1" id="form1">
                   <label for="mr2"></label>
                   <font color="#000000">
...
$sql = "SELECT count(*) FROM $dbbk.z_"."$ARCH"."_$tbl_fact where st='F'  and etat='$valeuretat'  ";  
...
$sql = "SELECT * FROM $dbbk.z_"."$ARCH"."_$tbl_fact where st='F'  and etat='$valeuretat' ORDER BY idf desc LIMIT ".$_GET['debut'].",".$nb_affichage_par_page;  
...
mysqli_free_result ($req); 
   echo '<span class="gras">'.barre_navigation($nb_total, $nb_affichage_par_page, $_GET['debut'],$valeuretat, $ARCH , 10).'</span>';  
}  
