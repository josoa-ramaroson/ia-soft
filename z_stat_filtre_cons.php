...
?>
<body>
 <p><a href="stat_filtre_consimp.php?an=<?php echo md5(microtime()).$annee1fl;?>&ns=<?php echo md5(microtime()).$nserie1;?>&CA=<?php echo md5(microtime()).$CA;?>&CB=<?php echo md5(microtime()).$CB;?>" target="_blank"><img src="images/imprimante.png" width="50" height="30"></a> 
...
{
$sql11 = "SELECT  COUNT(*) AS nbres, SUM(cons1) AS cons1, SUM(cons2) AS cons2, SUM(cons) AS cons, SUM(mont1) AS mont1,SUM(mont2) AS mont2,SUM(puisct) AS puisct, SUM(totalht) AS totalht, SUM(tax) AS tax, SUM(totalttc) AS totalttc, SUM(ortc) AS ortc, SUM(impayee) AS impayee, SUM(Pre) AS Pre, SUM(totalnet) AS totalnet, refcommune , nserie , fannee , Tarif , st FROM $tv_facturation where fannee='$annee1fl'  and nserie='$nserie1' and cons > '$CA' and  '$CB' > cons GROUP BY refcommune ";
}
else
{
...
$sql4 = "SELECT COUNT(*) AS nbtotal FROM z_"."$ARCH"."_$tbl_fact f , $tbl_contact c  where  c.id=f.id and f.fannee='$annee1fl'  and f.nserie='$nserie1'";
...
?>
  <tr>
    <td  bgcolor="#FFFFFF"><a href="stat_filtre_cons_liste.php?an=<?php echo md5(microtime()).$annee1fl;?>&ns=<?php echo md5(microtime()).$nserie1;?>&CA=<?php echo md5(microtime()).$CA;?>&CB=<?php echo md5(microtime()).$CB;?>"  class="btn btn-sm btn-success" target="_blank"> LA LISTE</a></td>
