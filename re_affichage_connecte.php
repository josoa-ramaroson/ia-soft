...
<body>
 <p>
En attente de Branchement</p>
 <p>
  <?php
$sql = "SELECT count(*) FROM $tbl_contact  where statut='4'";  
...
$sql = "SELECT * FROM $tbl_contact  where statut='4' ORDER BY nomprenom ASC LIMIT ".$_GET['debut'].",".$nb_affichage_par_page;  
...
?>
   <tr>
     ...
     <?php if (($_SESSION['niveau']==44) and ($data['statut']==4)) {?>
 <a href="re_affichage_confstatut.php?id=<?php echo md5(microtime()).$data['id']; ?>&satut=<?php $a='5';echo md5(microtime()).$a; ?>" onClick="return confirm('Etes-vous sûr que ce client est branché')" ; style="margin:5px"   class="btn btn-sm btn-danger" >Confirmation</a><?php } else { } ?>
     </td>
   </tr>
   <?php
}
